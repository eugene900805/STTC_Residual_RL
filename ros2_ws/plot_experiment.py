#!/usr/bin/env python3
"""Plot TB3 slip experiment under injected proportional slip (steps at 30/50 s):
+velocity inner loop vs RL (SAC) vs Residual RL, against the reference path.
Reads /tmp/tb3_exp.npz."""
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
plt.tight_layout()
plt.savefig('/home/eugene/STT/figures_tb3/slip_all.png', dpi=120)
for m in modes:
    print(f'{m:8s} steady RMSE = {rmse(m):.4f}')
print('saved figures_tb3/slip_all.png')
