...
class TB3Params:
    ...
    # re-tuned for the real (dynamic) TB3: stronger pose feedback (Hx,Hy) to
    # reject slip, much gentler adaptation (rho) so the slip estimate stays
    # stable despite the robot's velocity-tracking lag.
    Hx = 4.0
    Hy = 4.0
    Hs = 0.3
    lam = 0.5
    rho1 = 2.0
    rho2 = 1.0
    sigma = 1.0   # leakage (sigma-modification): robustness vs unmodelled lag
