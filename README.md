...
1. **復現論文控制器**(分層 backstepping + 滑移自適應),Python 與 MATLAB 交叉驗證;
   直線誤差較無補償降低約 **16×**(0.170→0.011 m)。
2. **四種 DRL 對照**(SAC/PPO/TD3/DDPG,相同條件):**SAC ≈ TD3 ≫ PPO ≫ DDPG**;
   在**圓形**上 SAC/TD3 甚至**勝過論文控制器**(0.095 / 0.098 vs 0.142 m)。
3. **TurtleBot3(Gazebo)真機部署**——直線 + 圓形、含滑移、10 Hz:

   | 控制器 | 直線 RMSE [m] | 圓形 RMSE [m] |
   |--------|:---:|:---:|
   | 純運動學 | 0.258 | 0.361 |
   | Gazebo-訓練 RL (SAC) | 0.309 ⚠️ | 0.090 |
   | 速度內環(模型基) | 0.030 | 0.028 |
   | **殘差 RL（速度內環＋RL 殘差）** | **0.0050** | **0.0069** |

   - **「演算法＋RL 殘差」在兩種軌跡都最佳**(0.005 / 0.007 m)。
   - **速度內環**是最穩健的單一方法(0.03 m),直線、圓形皆穩。
   - **純 SAC 策略在直線上失敗**(0.309 m,比純運動學還差)→ 印證「模型基為主力、
     純學習較不可靠」。
4. **sim-to-real 關鍵**:RL 要上真機需 (1) 在物理迴圈內訓練、(2) 訓練/部署控制率匹配
   (確定性 10 Hz);控制率不匹配時誤差達 2.27 m,匹配後 0.090 m。
...
- 控制率不匹配(訓練 20Hz / 部署 5Hz)時 RL 給 2.27 m;**匹配後(10Hz)給 0.090 m**。

## 真機(Gazebo,含滑移)四方對照 — 直線 + 圓形,10 Hz

| 控制器 | 直線 RMSE [m] | 圓形 RMSE [m] |
|--------|:---:|:---:|
| 純運動學 | 0.258 | 0.361 |
| Gazebo-訓練 RL (SAC) | 0.309 | 0.090 |
| 速度內環(模型基) | 0.030 | 0.028 |
| **殘差 RL(速度內環＋殘差)** | **0.0050** | **0.0069** |

對照圖:`figures_tb3/`(`slip_all.png` 圓形、`slip_all_line.png` 直線、
`residual_vs_base.png` 殘差、`slip_veloop.png` 速度內環效果)。

## 總工程結論
1. **速度內環 + 殘差 RL 最佳**:在直線、圓形上都最準(~0.005–0.007 m)且穩健。
2. **速度內環**(模型基)是最穩健的單一方法;**純 SAC** 在圓形可行(0.090),但**直線失敗**
   (0.309,比純運動學差)→ 純學習策略跨軌跡較不可靠。
3. **RL 要上真機**需 physics-in-the-loop 訓練(消 sim-to-real gap)+ **控制率匹配**。
4. 模擬層 RL 可勝過論文(圓形 0.095 vs 0.142),但真機層仍以**模型基 + 殘差**最可靠。
